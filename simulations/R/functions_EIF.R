...
estimate_martingales <- function(working_model, full_data, model_cov, corr_model = T){
  ...
  indicator_uncensored <- get_censor_status(full_data)
...
P_treatment_extend_survival <- function(model, max_time, model_cov){
  ...
  cumhaz_hat <- predict_cox.aalen(covar = covar_true, 
                                  betaHat = beta_hat, 
                                  cum_base_haz = cum_haz)[,1:jump_times_to_keep]
...
get_K_hat <- function(data, model, corr_model = T){
  ...
  cumhaz_cens_hat <- predict_cox.aalen(covar = covar_cens, betaHat = betahat_cens, cum_base_haz = cum_basehaz_cens)
...
EIF <- function(data, T_corr = T, Cens_corr = T, max_time = 5){
  ...
  if(T_corr){T_model <- oracle_model(data)}
  else{T_model <- non_oracle_model(data)}
  
  if(Cens_corr){C_model <- oracle_cens_model(data)}
  else{non_oracle_cens_model(data)}
  
  if(T_corr){
    T_model_cov <- get_oracle_covar(data)
  }else{T_model_cov <- get_n_oracle_covar(data)}
  ...
  K_C_hat <- get_K_hat(data = data, model = T_model, corr_model = Cens_corr)[,1:jump_times_to_keep]
